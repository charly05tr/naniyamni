...
export const ProveedorCard = ({ proveedor }) => {
    ...
    return (
        <button onClick={irADetalle} className="text-start bg-gradient-to-r p-[1px] dark:bg-transparent hover:from-blue-600/60 hover:to-yellow-300/50 hover:cursor-pointer group block w-full rounded-xl shadow-lg transition-transform transform hover:-translate-y-1 hover:shadow-2xl duration-300">
            <div className="bg-gray-50 flex flex-col gap-2 rounded-xl max-h-60  dark:bg-[#181818] dark:border-[#AAAAAA]/30 dark:border">
                <div className="flex flex-row items-center md:items-start md:gap-2">
                    <img src={imgURL} alt={proveedor.nombre} className="lg:h-60 lg:w-60 md:h-44 md:w-44 w-20 h-20 object-cover md:rounded-l-xl dark:opacity-85"/>
