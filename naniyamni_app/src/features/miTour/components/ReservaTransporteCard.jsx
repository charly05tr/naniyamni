...
export const ReservaTransporteCard = ({ reserva, eliminar, irAProveedor, handleOpen, inPay }) => {
  const {
    id,
    servicio,
    fecha_hora_salida,
  ...
  return (
    <div className="md:max-h-62 flex flex-col md:flex-row rounded-xl shadow-lg overflow-hidden md:max-w-3xl w-[92dvw] mx-auto transform transition-transform duration-200 hover:scale-101 dark:hover:shadow-lg hover:shadow-[#F4B731]/20 hover:shadow-lg dark:border-[#AAAAAA]/10 dark:border dark:hover:shadow-[#FFD84C]/10">
      ...
      <div className="p-6 md:p-8 flex-1">
        <div className="flex gap-2 justify-between items-center">
            ...
            <span className="text-gray-600/80 text-nowrap ml-2 dark:text-[#F9FAFB]/60">{cant_personas} {(cant_personas > 1)?"personas":"persona"}</span>
        </div>
        <p className="text-xl font-bold dark:text-[#00BFFF]/90 text-[#2CA6A4]/90 mb-2">C$ {total}</p>  

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-2 mb-4">
          <div className="flex items-start flex-col gap-1">
            <span className="text-sm font-medium text-gray-500 mr-2 dark:text-[#F9FAFB]/60">Origen:</span>
            <span className="text-sm text-gray-800 line-clamp-1 dark:text-[#F9FAFB]/90">{servicio.origen}</span>
